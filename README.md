...
```mermaid
flowchart LR
    C1[Client] --> LB[Load Balancer :8080]
    C2[Client] --> LB
    C3[Client] --> LB

    LB --> A[Backend A :8001 alive]
    LB --> B[Backend B :8002 alive]
    LB -. skipped .-> C[Backend C :8003 dead]

    HC[Health Checker every 10s timeout 500ms] -. check /status .-> A
    HC -. check /status .-> B
    HC -. check /status .-> C
